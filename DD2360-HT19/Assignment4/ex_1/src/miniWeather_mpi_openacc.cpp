...
int main(int argc, char **argv) {
  ...
  sim_time = 300;     //How many seconds to run the simulation
...

{
  ...
  // MAIN TIME STEP LOOP
  ////////////////////////////////////////////////////
  #pragma acc data		create(sendbuf_l[0:hs*nz*NUM_VARS]) \
				create(sendbuf_r[0:hs*nz*NUM_VARS]) \
  				create(recvbuf_l[0:hs*nz*NUM_VARS]) \
				create(recvbuf_r[0:hs*nz*NUM_VARS])
	{
...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  #pragma acc parallel loop      private(inds, indt, dt) \
                                 copy(state_out[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
                                 copyin(state_init[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
                                 copyin(tend[0:nx*nz*NUM_VARS])\
				 collapse(3)
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc parallel loop 	copyout(flux[0:(nx+1)*(nz+1)*NUM_VARS]) \
				copyin(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])\
				private( inds, r,u,w,t,p, stencil[4], d3_vals[NUM_VARS], vals[NUM_VARS], hv_coef)
  ...
  #pragma acc parallel loop 	copyout(tend[0:nx*nz*NUM_VARS]) \
				copyin(flux[0:(nx+1)*(nz+1)*NUM_VARS]) \
				private(indt, indf1, indf2, dx, nz, nx) \
				collapse(3)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
...
void compute_tendencies_z( double *state , double *flux , double *tend ) {
  ...
  //Use the fluxes to compute tendencies for each cell
  #pragma acc parallel loop 	copy(tend[0:nx*nz*NUM_VARS]) \
				copyin(flux[0:(nx+1)*(nz+1)*NUM_VARS]) \
				copyin(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
				private(inds, indt, indf1, indf2, dx, nz, nx, hs, grav) \
				collapse(3)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc parallel loop	present(sendbuf_l[0:hs*nz*NUM_VARS]) \
				present(sendbuf_r[0:hs*nz*NUM_VARS]) \
				copyin(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
				private(nz, hs, nx) \
				collapse(3)
  for (ll=0; ll<NUM_VARS; ll++) {
  ...
  #pragma acc update host(sendbuf_l, sendbuf_r)
  ...
  #pragma acc update device(recvbuf_l, recvbuf_r)

  //Unpack the receive buffers
  #pragma acc parallel loop	present(recvbuf_l[0:hs*nz*NUM_VARS]) \
				present(recvbuf_r[0:hs*nz*NUM_VARS]) \
				copyout(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
				private(nz, hs, nx) \
				collapse(3)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
  #pragma acc parallel loop	copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
				copyin(hy_dens_cell[0:(nz+2*hs)]) \
				copyin(hy_dens_theta_cell[0:(nz+2*hs)]) \
				private(ind_r, ind_u, ind_t, nz, hs, nx) \
				collapse(2)
      for (k=0; k<nz; k++) {
        for (i=0; i<hs; i++) {
          z = (k_beg + k+0.5)*dz;
...
void set_halo_values_z( double *state ) {
  ...
  #pragma acc parallel loop	copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
				private(x, xloc, mnt_deriv, mnt_width, nz, hs, nx) \
				collapse(2)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
