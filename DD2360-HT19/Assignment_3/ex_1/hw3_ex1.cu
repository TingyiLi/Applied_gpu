...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    const int offset = row * (width*3) + col*3;
    image_out[row*width + col] = 0.0722f * image[offset] + 0.7152f * image[offset + 1] + 0.2126f * image[offset + 2];
}
...
__device__ float gpu_applyFilter_shared(float *sh_block, int shared_offset, int stride, float *matrix, int filter_dim)
{
    ////////////////
    // TO-DO #5.2 ////////////////////////////////////////////////
    // Implement the GPU version of cpu_applyFilter()           //
    ...
    for (int h = 0; h < filter_dim; h++)
    {
        int offset        = shared_offset + h * stride;
        int offset_kernel = h * filter_dim;

        for (int w = 0; w < filter_dim; w++)
        {
            pixel += sh_block[offset + w] * matrix[offset_kernel + w];
        }
    }
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (height - 2) && index_y < (width - 2))
    {
	...
	__syncthreads();
	int shared_offset = threadIdx.x * BLOCK_SIZE_SH + threadIdx.y;
        image_out[offset] = gpu_applyFilter_shared(sh_block, shared_offset, BLOCK_SIZE_SH, gaussian, 3);
    }
}
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (height - 2) && index_y < (width - 2))
    {
        ...
        __syncthreads();
        int shared_offset = threadIdx.x * BLOCK_SIZE_SH + threadIdx.y;
        float gx = gpu_applyFilter_shared(sh_block, shared_offset, BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter_shared(sh_block, shared_offset, BLOCK_SIZE_SH, sobel_y, 3);
...
int main(int argc, char **argv)
{
    ...
    grid       = dim3(((bitmap.height  + (BLOCK_SIZE - 1)) / BLOCK_SIZE),
                      ((bitmap.width + (BLOCK_SIZE - 1)) / BLOCK_SIZE));
